...
# jupyter nbconvert --to script /Users/ren/Projects-NeuroCode/MyExperiment/Aging-SeqMemTask/fMRI-code/AgingReplay_LocalizerDecoding_EightCatetory_SeqAnal_ROILoop_HPC.ipynb
# ========================================================================================================================================================
# HPC-ready version:
...
# ## Replay analysis: applying the trained classifier from the localizer task to the online replay period in the SeqMemTask

# ### Method 2: 8 items and 8 positions; Generalized decoding probability for every category of each TR in the main task (SeqMemTask)
# 
# Using the LogisticRegression in the scikit learn 
...
for i_roi in range(0, len(ROI_names)): # 
    ...
    for subIdx in range(6, subLen): # range(6, subLen)
        ...
        # ********** Step 1: Using the fMRI data from the localizer task to train the classifer (without cross-validation) **********
        # --------- Specify Decoder Parameters ---------
        # Initialize the decoder
        decoder_con = LogisticRegression(
            penalty='l2',
            C = 1.0, # inverse of regularizaiton strength; must be a positive float. default = 1.0
            solver = 'lbfgs', # default; multinomial multiclass
            multi_class = 'auto', # default, 'auto' selects 'ovr' if the data is binary, or if solver='liblinear', and otherwise selects 'multinomial'
            n_jobs = -1
        )

        decoder_pos = LogisticRegression(
            penalty='l2',
            C = 1.0, # inverse of regularizaiton strength; must be a positive float. default = 1.0
            solver = 'lbfgs', # default; multinomial multiclass
        ...
        # Assuming you already have a NiftiMasker with the same mask used in preprocessing
        # Example: masker = NiftiMasker(mask_img=mask_img, standardize=True, t_r=t_r)
        # Fit the masker to the original mask
        # masker_train = masker
        # masker_train.fit()

        # Transform the 2D masked data (n_samples, n_voxels) back to 4D Nifti image
        # nifti_data_con = masker_train.inverse_transform(masked_data_con)
        # nifti_data_pos = masker_train.inverse_transform(masked_data_pos)

        # --------- Fit the Decoder ---------
        decoder_con.fit(X = masked_data_con, y = conditions_con_label) # X: (n_samples, n_features/n_voxels), y: (n_samples,)
        decoder_pos.fit(X = masked_data_pos, y = conditions_pos_label) 

        # --------- Retrieve Decoding Weights ---------
        # Access weights for each class
        decoding_weights_con = decoder_con.coef_ # (1, n_features) or (n_classes, n_features)
        decoding_weights_pos = decoder_pos.coef_

        if standardization_word == 'zscore':
            # --------- Standardize the classifier weights ---------
            # Standardize each row: the classifier weight for each category
            standardized_data = np.array([scaler.fit_transform(row.reshape(-1, 1)).flatten() for row in decoding_weights_con])
            decoder_con.coef_ = standardized_data

            standardized_data = np.array([scaler.fit_transform(row.reshape(-1, 1)).flatten() for row in decoding_weights_pos])
            decoder_pos.coef_ = standardized_data

        # ********** Step 2: Apply the trained classifier to the fMRI data in the SeqMemTask **********
        # +++++++++ Load fMRI data from the main task (SeqMemTask) and apply the trained classifier +++++++++++
        
        predictionProb_con_iSub = np.zeros((36, 8, nTRanal)) # 36: 6 blocks with 6 trials of long WTI; 8: 8 items or 8 positions
        predictionProb_pos_iSub = np.zeros((36, 8, nTRanal))
        for i_tr in range(0, nTRanal):
            ...
            # i.e., calculate the probability of each class assuming it to be positive using the logistic function and normalize these values across all the classes.
            # --------The decoding probability: trialNo X categories--------
            predictionProb_con = decoder_con.predict_proba(nifti_data_smt_masked) # the decoding probability for each category
            predictionProb_pos = decoder_pos.predict_proba(nifti_data_smt_masked) 

            predictionProb_con_iSub[0 : np.shape(predictionProb_con)[0], :, i_tr] = predictionProb_con
            predictionProb_pos_iSub[0 : np.shape(predictionProb_pos)[0], :, i_tr] = predictionProb_pos
        
        # If no blocks were deleted, the output should be equal 6(block) * 6(longWTI trial)
        predictionProb_con_iSub = predictionProb_con_iSub[0 : np.shape(predictionProb_con)[0], :, :]
        predictionProb_pos_iSub = predictionProb_pos_iSub[0 : np.shape(predictionProb_pos)[0], :, :]
        
        # ----------Reorganize the 3-D array into 2-D----------
        trialNo_iSub = np.shape(predictionProb_con_iSub)[0]
        predictionProb_con_iSub_flat = np.zeros((trialNo_iSub * nTRanal, 8))
        predictionProb_pos_iSub_flat = np.zeros((trialNo_iSub * nTRanal, 8))
        i_row = 0
        for i_tr in range(0, nTRanal):
            for i_trl in range(0, trialNo_iSub):
                # ------decodeProb for the items------
                predictionProb_con_ij = np.squeeze(predictionProb_con_iSub[i_trl, :, i_tr])
                predictionProb_con_iSub_flat[i_row, :] = predictionProb_con_ij

                # ------decodeProb for the positions------
                predictionProb_pos_ij = np.squeeze(predictionProb_pos_iSub[i_trl, :, i_tr])
                predictionProb_pos_iSub_flat[i_row, :] = predictionProb_pos_ij

                i_row = i_row + 1

        # ----------Convert np.array to dataframe----------
        predictionProb_con_iSub_pd = pd.DataFrame(predictionProb_con_iSub_flat)
        predictionProb_pos_iSub_pd = pd.DataFrame(predictionProb_pos_iSub_flat)

        # ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        # ----------Save the predicted probabilities----------
        predictCon_filename = smtDatas_saveDir + '/' + subID_num + '-' + subID_str + '-' + ROI_name + '_space-' + space + '_signalT-' + str(int(T*100)) + '-' + standardization_word + "-predictionProb_con_iSub_pd.csv"
        predictionProb_con_iSub_pd.to_csv(path_or_buf = predictCon_filename) 

        predictPos_filename = smtDatas_saveDir + '/' + subID_num + '-' + subID_str + '-' + ROI_name + '_space-' + space + '_signalT-' + str(int(T*100)) + '-' + standardization_word + "-predictionProb_pos_iSub_pd.csv"
        predictionProb_pos_iSub_pd.to_csv(path_or_buf = predictPos_filename) 


print("\nAll generalized decoding runs completed successfully.\n")
