#!/bin/bash
#SBATCH --job-name=reliabilityReplay_%A_%a
#SBATCH --output=/home/mpib/ren/logs/reliabilityReplay_%A_%a.out
#SBATCH --error=/home/mpib/ren/logs/reliabilityReplay_%A_%a.err
...
# Submit the bash file: TOP_FRAC=0.30 sbatch AgingReplay_LocalizerDecodingReliability_SeqAnal_submit.sh
...
echo "Reliability-based generalized decoding for ROI: $ROI_NAME"
echo "Task ID: $SLURM_ARRAY_TASK_ID"
echo "top-frac: $TOP_FRAC"

# --- Run Python script with ROI name ---
python AgingReplay_LocalizerDecoding_EightCatetory_SeqAnal_ROILoop_HPC.py \
  --roi "$ROI_NAME" \
  --top-frac "$TOP_FRAC"
